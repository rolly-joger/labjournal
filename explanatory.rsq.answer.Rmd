...
```{r good data}
the_data <- sienaDataCreate(net, white_pass_covar, country_covar, gender_covar, functie_covar, h_index_covar, interaction_race_covar, interaction_ethnicity_covar)
myeff_1 <- getEffects(the_data)
view(myeff_1)
myeff_1 <- includeEffects(myeff_1, X, interaction1 = "white_pass_covar")
...
myeff_1 <- includeEffects(myeff_1, Jin)
## Race Effects
```

### Results
```{r results alogoritm 2, echo = F}
myAlgorithm <- sienaAlgorithmCreate(projname = "preliminary")
mod_1_effects <- siena07(myAlgorithm, data = the_data, effects = myeff_1, returnDeps = TRUE)
mod_1_effects 
```
...
myeff_1 <- includeEffects(myeff_1, egoX, altX, interaction1 ="white_pass_covar")
myeff_1 <- includeEffects(myeff_1, inpopX, interaction1 ="white_pass_covar")
myeff_1 <- includeEffects(myeff_1, sameX, interaction1 = "gender_covar")
myeff_1 <- includeEffects(myeff_1, egoX, altX, interaction1 = "gender_covar")
...
myeff_1 <- includeEffects(myeff_1, sameX, interaction1 = "functie_covar")
myeff_1 <- includeEffects(myeff_1, egoX, altX, interaction1 = "functie_covar")
```

### Results
```{r results alogoritm 5, echo = F}
myAlgorithm <- sienaAlgorithmCreate(projname = "preliminary")
mod_1_effects <- siena07(myAlgorithm, data = the_data, effects = myeff_1, returnDeps = TRUE)
mod_1_effects 
```
...
### Results
```{r results alogoritm 8, echo = F}
myAlgorithm <- sienaAlgorithmCreate(projname = "preliminary")
mod_1_effects <- siena07(myAlgorithm, data = the_data, effects = myeff_1, returnDeps = TRUE)
mod_1_effects 
```
## Ethnicity and Structural
### Effects
```{r}
myeff_1 <- getEffects(the_data)
myeff_1 <- includeEffects(myeff_1, inPopSqrt)
...
# GOF
### Create Effects
```{r GOF functions Model 1-ish, message = F, warning=F}
gofi0_1 <- sienaGOF(mod_1_effects, IndegreeDistribution, verbose = FALSE, join = TRUE, varName = "net")
gof2 <- sienaGOF(mod_1_effects, auxiliaryFunction = TriadCensus, verbose = F, join = TRUE, varName = "net")
```
